...
public class BlockTeleporterBlockEntity extends RandomizableContainerBlockEntity implements WorldlyContainer {
	private NonNullList<ItemStack> stacks = NonNullList.withSize(1, ItemStack.EMPTY);

	public BlockTeleporterBlockEntity(BlockPos position, BlockState state) {
		super(CreateteleportersModBlockEntities.BLOCK_TELEPORTER.get(), position, state);
	}
	...
	public Component getDefaultName() {
		return Component.literal("block_teleporter");
	}

	@Override
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new BlockTeleporterGuiMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
